class Quesion_Paser:
    def build_entitydict(self, args):
        ...
        for arg, types in args.items():
            for type in types:
                ...
                '''
        将{'内科': ['department'], '肺炎': ['disease'], '阿莫西林': ['drug']}
        转换为
          {'department': ['内科'], 'disease': ['肺炎'], 'drug': ['阿莫西林']}
        '''
        return entity_dict
    ...
    def parser_main(self,res_classify):
            ...
            sqls = []
            # question_types = ['symptom_disease', 'disease_not_food']之类的
    ...
    def sql_transfer(self, question_type, entities):
        ...
        # 查询疾病的持续时间
            ...
            sql = ["MATCH (m:Disease) where m.name = '{0}' return m.name, m.treatment_cycle".format(i) for i in entities]

        # 查询疾病的治愈概率
        elif question_type == 'disease_cureprob':
            sql = ["MATCH (m:Disease) where m.name = '{0}' return m.name, m.cure_rate".format(i) for i in entities]
        ...
        # 查询疾病建议吃的东西
            ...
            sql = ["MATCH (m:Disease)-[r:do_eat]->(n:Food) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
        ...
        # 已知推荐查疾病
            ...
            sql = ["MATCH (m:Disease)-[r:do_eat]->(n:Food) where n.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
        ...
        # 已知药品查询能够治疗的疾病
            ...
            sql= ["MATCH (m:Disease)-[r:recommand_drug]->(n:Drug) where n.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
        ...
        '''
rel_classify
{'args': {'内科': ['department'], '肺炎': ['disease'], '阿莫西林': ['drug']},
'question_types': ['disease_prevent', 'disease_cureway']}


parser_main(rel_classify)

[{'question_type': 'disease_prevent',
'sql': ["MATCH (m:Disease) where m.name = '肺炎' return m.name, m.prevent"]},
{'question_type': 'disease_cureway',
'sql': ["MATCH (m:Disease) where m.name = '肺炎' return m.name, m.cure_way"]}]
